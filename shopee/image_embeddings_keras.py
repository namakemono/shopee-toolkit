...
def to_images(filepath_list, image_size):
    res = []
...
def predict_generator(df:pd.DataFrame, image_size:int, batch_size:int=8):
    n = len(df)
    for i in range(0, n, batch_size):
        filepath_list = df.iloc[i:i+batch_size]["filepath"].tolist()
        images = to_images(filepath_list, image_size)
        yield images

def get_efficientnet_backbone():
    base_model = tf.keras.applications.EfficientNetB3(include_top=False,
        weights=None, input_shape=(None, None, 3)) # can pretrain with ImageNet
